...
<div class="container">
	...
	<div
		...
		<div class="col-sm-5 col-lg-3 m-0" style="cursor: pointer;"
			onclick='outToExcel( <?php echo isset($pageData["all_order_data"]) ? json_encode($pageData["all_order_data"]) : "[]"; ?>)'>
				...
				<div class="filter-popup" id="filter-guys-popup" style="z-index: -1;">
					...
					<form action="" method="POST">
						<div class="" style="direction: rtl;">
							...
							<select name="stores_array[]" type="text" class="form-select" placeholder="Select tags"
							...
							<input type="hidden" name="filter_by_stores">
							<button type="submit" class="btn btn-primary bg-black mt-3">Filter</button>
						</div>
					</form>
	...
	<div class="d-flex flex-row gap-3 mt-3 flex-wrap" style="height: fit-content; overflow-y: auto;">
		<?php foreach ($pageData['all_order_data'] as $order_data) {
			...
			if ($order_data['order_status'] == ORDER_STATUS_PAID) {
				...
				$order_data['order_status'] == ORDER_STATUS_PENDING ||
			...
			<div class="card order-management-cards col-xl-4 rounded-3">
				...
				<div class="card-body px-3">
					...
					<p class="card-footer-text pt-4 pb-2">סה”כ הזמנה:
						<?php echo $order_data['order_total']; ?> ₪
					</p>
				</div>
			</div>
		<?php } ?>

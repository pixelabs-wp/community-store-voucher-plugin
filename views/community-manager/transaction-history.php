  ...
  <div class="container col-12 mt-5">
    ...
    <div
      ...
      <div class="col-sm-5 col-lg-3 m-0" style="cursor: pointer;"
        onclick='outToExcel( <?php echo isset($pageData["amount_transaction"]) ? json_encode($pageData["amount_transaction"]) : "[]"; ?>)'>
            ...
            <div class="" style="direction: rtl;">
              <form action="" method="POST">
                ...
                <select type="text" class="form-select" placeholder="Select tags" id="guys-select-tags" value=""
                  multiple>
                  <?php
								foreach ($pageData["voucher_transaction"] as $data) {
									
										$guy_name = $data['full_name'];
										?>
										<option value="<?php echo $guy_name; ?>">
											<?php echo $guy_name; ?>
										</option>
									<?php
                ...
                foreach ($pageData["amount_transaction"] as $data) {
            ...
            if (isset($pageData["voucher_transaction"])) {
              foreach ($pageData["voucher_transaction"] as $transaction) {
                ...
                <tr>
                  ...
                  <td class="text-muted ts-store-name">שם חנות:
                    <?php echo $transaction['store_data']->store_name; ?>
                  </td>
                  <td class="ts-guy-name">שם הבחור:
            ...
            if (isset($pageData["amount_transaction"])) {
              foreach ($pageData["amount_transaction"] as $amount) {
                ...
                <tr>
                  ...
                  <td class="text-muted ts-price">
                    <?php echo $amount['transaction_amount']; ?> ₪
                  </td>
                  <td class="text-muted ts-product">
                    מוצר:
                    <?php echo $amount['transaction_type'] ?>
                  </td>
                  <td class="text-muted ts-store-name">Transaction</td>
                  <td class="ts-guy-name">שם הבחור:
                    <?php echo $amount['member_data']->full_name; ?>
                  </td>
                </tr>
              <?php }
            } ?>
